...
export const mealEvents = sqliteTable("meal_events", {
  ...
  // A meal event is exactly ONE kind (app-enforced): a recipe meal (recipeId),
  // a direct ingredient (ingredientId + amount), or a direct product (productId,
  // optional variantId, + amount). recipeId is nullable so direct items don't
  // need a fake one-ingredient recipe.
  recipeId: integer("recipe_id").references(() => recipes.id),
  servings: integer("servings").notNull().default(1),
  // direct item: a plain ingredient, or a product (with optional pack variant)
  ingredientId: integer("ingredient_id").references(() => ingredients.id),
  productId: integer("product_id").references(() => products.id),
  variantId: integer("variant_id").references(() => productVariants.id),
  // canonical units consumed by a direct item (null for recipe meals)
  amount: real("amount"),
  status: text("status").notNull().default("planned"), // 'planned' | 'cooked' | 'served'
  // true when stock was depleted by an explicit "cook ahead" (not by serving).
  // Lets un-serve return to the right prior state: 'cooked' if cooked ahead,
  // else all the way back to 'planned' (backing the serve's stock out too).
  cookedAhead: integer("cooked_ahead", { mode: "boolean" }).notNull().default(false),
