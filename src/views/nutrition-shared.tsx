...
export interface MealLine { eventId: number; slotName: string; recipeName: string; estimate: boolean; calories: number; nutrients: Nutrients; }
...
// Date state that starts empty (so SSR and first client render agree on the
...
// Fetches GET /api/nutrition/analysis for the given mode/date and derives
// loading + noMeals. Only mounts once per active view, so mobile and desktop
// each fetch a single time (the switcher only renders one).
export function useNutritionAnalysis(mode: "day" | "week", date: string) {
...
export function OverviewBody({ data, mode, openCard, setOpenCard }: {
...
export function BreakdownBody({ data, mode, date }: { data: AnalysisData; mode: "day" | "week"; date: string }) {
