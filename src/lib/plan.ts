...
/**
 * Delete a planned event. If it was generated by a rule, tombstone the day so
 * the rule won't regenerate it. Cooked events are kept (stock already moved).
 */
export function deleteEvent(db: Db, householdId: number, eventId: number) {
  ...
  if (ev.ruleId) {
    skipDay(db, ev.ruleId, ev.date, ev.slotId);
  } else {
    db.delete(schema.mealEvents).where(eq(schema.mealEvents.id, ev.id)).run();
  }
}
